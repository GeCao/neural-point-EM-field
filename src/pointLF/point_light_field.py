...
from .pointcloud_encoding.pointnet_features import (
    PointNetDenseCls,
...
class PointLightField(nn.Module):
    def __init__(
        ...
        if feature_encoder == 'pointnet_segmentation':
            self._PointFeatures = PointNetDenseCls(
                k=n_pt_features, feature_transform=feature_transform
            )
        elif feature_encoder == 'pointnet_lf_pt_only':
            self._PointFeatures = PointNetLightFieldEncoder(
                k=n_pt_features, feature_transform=feature_transform, points_only=True
            )  # 1088 = 1024 + 64
        elif feature_encoder == 'pointnet_lf_global':
            self._PointFeatures = PointNetLightFieldEncoder(
                k=n_pt_features, feature_transform=feature_transform, points_only=False
            )  # 1024
            ...
            self._PointFeatures = MVModel(
                task='cls', backbone='resnet18', feat_size=16, resolution=512
            )
            self.n_pt_features = 128
            self.key_len = 64
            self.pre_scale = True
        ...
        if self.sky_dome:
            self._sky_latent = nn.Parameter(
                torch.rand(self.n_pt_features), requires_grad=True
            )
            self._SkyField = LightFieldNet(
                n_feat_in=n_feat_in,
                n_pt_feat=self.n_pt_features,
    ...
    def forward(
        ...
        if self.training or self.first_val_pass:
            output_dict = {
                'points_in': x.detach().cpu().numpy().copy(),
                'points_selected_in': np.stack(
                    [
                        pts[mask].detach().cpu().numpy()
                        for (pts, mask) in zip(x, closest_mask)
                    ]
                ),
                'rays_in': ray_dirs.detach().cpu().numpy().copy(),
                'closest_mask_in': closest_mask.detach().cpu().numpy().copy(),
                'samples': np.stack([np.array(smpl_id)[:2] for smpl_id in sample_idx])[
                    :, None
                ],
            }
        else:
        ...
        if self._k_closest:
            ...
            if not self.no_feat:
                if self.pre_scale:
                    ...
                    feat = [
                        pts_feat[mask] for (pts_feat, mask) in zip(feat, closest_mask)
                    ]
            ...
            if self.feat_weighting == 'lin':
                ...
                feat = torch.zeros(
                    batchsize, n_batchrays, self._k_closest, 1, self.n_pt_features
                )
                feat_sel_proj = feat.detach().cpu().numpy().copy()
                feat, attn_weights = self.AttentionModule(
                    ...
                    features=feat,
                    distance=x_dist,
                    projected_distance=x_proj,
                    pitch=x_pitch,
                    azimuth=x_azimuth,
                )
                output_dict.update(
                    {
                        'attention_weights': attn_weights.view(
                            batchsize, n_batchrays, -1, self._k_closest
                        )
                        .cpu()
                        .detach()
                        .numpy()
                ...
                if not self.new_enc and not self.sky_dome:
                    feat, attn_weights = self.AttentionModule(
                        ray_dirs, feat, x_proj, x_dist, pitch=x_pitch, azimuth=x_azimuth
                    )
                    output_dict.update(
                        {
                            'attention_weights': attn_weights.view(
                    ...
                    if len(point_mask[0]) > 0:
                        point_feat, point_attn_weights = self.AttentionModule(
                            directions=ray_dirs[point_mask][None],
                    ...
                    feat = torch.zeros(
                        [batchsize, n_batchrays, self.n_pt_features], device=x.device
                    )
                    # Store attention weights for debugging/visualization
                    attn_weights = torch.zeros(
                        [batchsize, n_batchrays, self._k_closest + 1], device=x.device
                    )
                    ...
                    if len(point_mask[0]) > 0:
                        ...
                        point_attn_weights = torch.cat(
                            [
                                point_attn_weights,
                                torch.zeros(
                                    [len(point_mask[0]), 1, 1], device=x.device
                                ),
                            ],
                            dim=-1,
                        )
                    ...
                    output_dict.update(
                        {
                            'attention_weights': attn_weights.view(
                                batchsize, n_batchrays, -1, self._k_closest + 1
                            )
                            .cpu()
                            .detach()
                            .numpy()
                ...
                single_idx = tuple(
                    [
                        torch.linspace(0, batchsize - 1, batchsize, dtype=torch.int64)[
                            :, None
                        ].repeat(1, n_batchrays),
                        torch.linspace(
                            0, n_batchrays - 1, n_batchrays, dtype=torch.int64
                        )[None, :].repeat(batchsize, 1),
            ...
            feat = torch.zeros(
                list(ray_dirs.shape[:2]) + [128], dtype=x.dtype, device=x.device
            )
    ...
    def add_sky_features(
        ...
        sky_dist = torch.cat(
            [
                ...
                torch.zeros([1, n_sky_rays, 1], device=device),
            ],
            dim=-1,
        )
        sky_proj = torch.cat(
