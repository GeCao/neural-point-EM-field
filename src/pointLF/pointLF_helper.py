...
def pre_scale_MV(x, translation=-1.4):
    ...
    scaled_x = torch.minimum(scaled_x, torch.tensor([1.0], device="cuda"))
    scaled_x = torch.maximum(scaled_x, torch.tensor([-1.0], device="cuda"))
...
def select_Mv_feat(
    ...
    feature_maps = feature_maps.reshape(
        n_feat_maps, n_features, feat_heigth, feat_width
    )
    ...
    coordinates, coord_x, coord_y, depth = feature_extractor._get_img_coord(
        masked_scaled_pts, resolution=img_resolution
    )

    # Adjust for downscaled feature maps
    coord_x = torch.round(coord_x.view(n_feat_maps, -1, k_closest) / feat2img_f).to(
        torch.long
    )
    coord_x = torch.minimum(
        coord_x, torch.tensor([feat_heigth - 1], device=coord_x.device)
    )
    coord_y = torch.round(coord_y.view(n_feat_maps, -1, k_closest) / feat2img_f).to(
        torch.long
    )
    coord_y = torch.minimum(
        coord_y, torch.tensor([feat_width - 1], device=coord_x.device)
    )
    ...
    pts_feat = torch.stack(
        [feature_maps[i][tuple([coord_x[i], coord_y[i]])] for i in range(n_feat_maps)]
    )
...
def lin_weighting(z, distance, projected, my=0.9):
    ...
    pt_ray_dist_weights = (
        inv_pt_ray_dist / torch.norm(inv_pt_ray_dist, dim=-1)[..., None]
    )
    ...
    z = (
        z
        * (my * pt_ray_dist_weights + (1 - my) * pt_proj_dist_weights)[..., None, None]
    )
