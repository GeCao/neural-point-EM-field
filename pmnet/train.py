...
def train(model, train_loader, test_loader, optimizer, scheduler, writer, cfg=None):
    ...
    for epoch in range(cfg.num_epochs):
        ...
        for inputs, targets in tqdm(train_loader):
            ...
            if count % 100 == 0:
                print(
                    f"Epoch:{epoch}, Step:{count}, Loss:{loss.item():.6f}, BestVal:{best_val:.6f}, Time:{time.time()-tic}"
                )
            tic = time.time()
        ...
        if epoch % cfg.val_freq == 0:
            val_loss, best_val = eval_model(
                model, test_loader, error="MSE", best_val=best_val, cfg=cfg
            )
...
def eval_model(model, test_loader, error="MSE", best_val=100, cfg=None):
    ...
    if avg_loss < best_val:
        ...
        torch.save(
            model.state_dict(),
            f"{RESULT_FOLDER}/{cfg.exp_name}_epoch{cfg.num_epochs}/{cfg.param_str}/model_{best_val:.5f}.pt",
        )
        print(
...
if __name__ == "__main__":
    ...
    os.makedirs(
        f"{RESULT_FOLDER}/{cfg.exp_name}_epoch{cfg.num_epochs}/{cfg.param_str}",
        exist_ok=True,
    )
    ...
    if cfg.sampling == "exclusive":
        ...
        if "usc" in args.config.lower():
            ...
            data_train = PMnetEtoiCenter(is_train=True, dir_dataset=args.data_root)
    ...
    scheduler = optim.lr_scheduler.StepLR(
        optimizer, step_size=cfg.step, gamma=cfg.lr_decay
    )

    best_val = train(
        model, train_loader, test_loader, optimizer, scheduler, writer, cfg=cfg
    )
